...
def Crowding(M,phiE,beta,rhoE,d,switch):
    rhoM=1.8*10**5*0.55 #cytoplasm protein dry mass density
    if switch:
        V = (M/rhoM)
        S = (phiE*M/rhoE)
        num=beta*np.exp(-(M/V)/rhoM)
        denom=(3*V/S - d)**2  #denoting radius
        if denom==0:
            return 0
            print('Division by 0')
        else:
...
def FluxBalanceGlucose(M,phiE,phiR,phiRMin,phiF,phiO,JOut,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,switch):
    return JIn(M,phiE,G0,P,rhoE)-JF(M,phiE,phiF,beta,rhoE,d,epsilonF,switch)/eF-JB(M,phiE,phiR,phiRMin,beta,rhoE,d,fC,kt,switch)
...
def G0Optimizer(args,res,G0min,G0max,phiEmin,phiEmax,Mmin,Mmax):       
    ...
    for G0 in np.linspace(start=G0min, stop=G0max, num=res):
        ...
        #optimizedNormal(solNormal,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac)<optimizedOverflow(solOverflow,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac):
            ...
            sols.append([G0,kt*(phiR-phiRMin)*Crowding(solOverflow[0],solOverflow[1],beta,rhoE,d),solOverflow[0],solOverflow[1],phiR,phiF,phiO,JOut,Crowding(solOverflow[0],solOverflow[1],beta,rhoE,d)])
    return sols
...
def optimizedNormalV(x,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac,switch,kappaTarget):
    M=x[0]
    phiE=(160/M)**(1/3)
    #phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac = args
    argz = [M,phiE,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac,switch]
    ...
    phiF=PhiFBalanced(argz,False)
    phiO=PhiOBalanced(argz)
    if phiR+phiF+phiE+phi0>1 or phiR<0 or phiF<0 or phiO<0 or kt*(phiR-phiRMin)*Crowding(M,phiE,beta,rhoE,d,switch)<kappaTarget:
        return 10000
    return -((M*phiE/rhoE)**3/(36*np.pi))**0.5

def optimizedOverflowV(x,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac,switch,kappaTarget):
    M=x[0]
    phiE=(160/M)**(1/3)
    gamma_eff = phiOMaxFrac*(1 - f_in)
    argz = [M,phiE,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,gamma_eff,switch]
    ...
    phiO=gamma_eff*phiE
    JOut=JOutBalanced(argz)
    if phiR+phiF+phiE+phi0>1 or phiR<0 or phiF<0 or JOut<0 or kt*(phiR-phiRMin)*Crowding(M,phiE,beta,rhoE,d,switch)<kappaTarget:
        return 10000
    return -((M*phiE/rhoE)**3/(36*np.pi))**0.5  

def G0OptimizerV(args,res,G0min,G0max,Mmin,Mmax):       
    phiRMin,phi0,a,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac,switch,kappaTarget = args
    #bounds=Bounds([Mmin,Mmax],[phiEmin,phiEmax])
    sols=[]
    for G0 in np.linspace(start=G0min, stop=G0max, num=res):
        argz=(phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac,switch,kappaTarget)
        #solNormal=minimize(optimizedNormal, [1.7*10**5*0.55,(phiEmin+phiEmax)/2], args=argz,
        #                   method='BFGS', bounds=((Mmin,Mmax),(phiEmin,phiEmax)))
        #solOverflow=minimize(optimizedOverflow, [1.7*10**5*0.55,(phiEmin+phiEmax)/2], args=argz,
        #                   method='BFGS', bounds=((Mmin,Mmax),(phiEmin,phiEmax)))
        ranges=(slice(Mmin,Mmax,(Mmax-Mmin)/10000),)
        solNormal=brute(optimizedNormalV,ranges, args=argz,finish=None,full_output=False)#2500,Ns=100
        phiE=(160/solNormal)**(1/3)
        #print(solNormal)
        gamma_eff = phiOMaxFrac*(1 - f_in)
        if PhiOBalanced([solNormal,phiE,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac,switch])<gamma_eff*phiE:
        #optimizedNormal(solNormal,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac)<optimizedOverflow(solOverflow,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac):
            argzz=[solNormal,phiE,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,phiOMaxFrac,switch]
            phiR=PhiRBalanced(argzz,False)
            phiF=PhiFBalanced(argzz,False)
            phiO=PhiOBalanced(argzz)
            sols.append([G0,kt*(phiR-phiRMin)*Crowding(solNormal,phiE,beta,rhoE,d,switch),solNormal,phiE,phiR,phiF,phiO,0,Crowding(solNormal,phiE,beta,rhoE,d,switch),efficiency(solNormal,phiE,phiR,phiRMin,phiF,phiO,0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,switch)])
        else:            
            solOverflow=brute(optimizedOverflowV,ranges,args=argz,finish=None,full_output=False)
            phiE=(160/solOverflow)**(1/3)
            argzz_ov=[solOverflow,phiE,phiRMin,phi0,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,gamma_eff,switch]
            phiR=PhiRBalanced(argzz_ov,True)
            phiF=PhiFBalanced(argzz_ov,True)
            phiO=gamma_eff*phiE
            JOut=JOutBalanced(argzz_ov)
            sols.append([G0,kt*(phiR-phiRMin)*Crowding(solOverflow,phiE,beta,rhoE,d,switch),solOverflow,phiE,phiR,phiF,phiO,JOut,Crowding(solOverflow,phiE,beta,rhoE,d,switch),efficiency(solOverflow,phiE,phiR,phiRMin,phiF,phiO,JOut,a,G0,P,epsilonF,epsilonO,eF,eO,eB,eP,beta,rhoE,rhoP,d,fC,kt,switch)])
    return sols
...
def ProteomePlot(sols,runName):
    ...
    #plt.axvspan(8806.86, np.max(G0List), facecolor='0.05', alpha=0.1)
...
def ProteomePlotKappa(sols,runName):
    ...
    plt.margins(x=0,y=0.05)
    plt.legend([r'$\phi_R$',r'$\phi_E$',r'$\phi_G$',r'$\phi_O$',r'$\phi_0$'],loc='upper right')
...
def EfficiencyPlot(sols,runName):
    ...
    plt.xlabel(r'$[G_0]$ $[\mu \mathrm{M}$',fontsize=18)
...
def EnergyPlot(sols,args,runName):
    ...
    plt.legend(['Glycolytic','Respiration','Acetate','Lipid Synthesis','Maintenance','Translation'],loc='lower left',fontsize=12)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.xlabel(r'Ext. glucose conc. $[G_0]$ $[\mu \mathrm{M}$',fontsize=18)
...
def VPlot(sols,args,runName):
    ...
    plt.xlabel(r'Ext. glucose conc. $[G_0]$ $[\mu m^{-3}]$',fontsize=18)
